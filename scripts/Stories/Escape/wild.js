...
if (!Server.settings.getScore('wildMax'))
    Server.settings.setScore('wildMax', 10000);
if (!Server.settings.getScore('wildMin'))
    Server.settings.setScore('wildMin', 300);
if (!Server.settings.getScore('wildOriginX'))
    Server.settings.setScore('wildOriginX', 0);
if (!Server.settings.getScore('wildOriginZ'))
    Server.settings.setScore('wildOriginZ', 0);
Server.command.register({
    ...
    if (!Server.basicToggleChecker('wild', chatmsg.sender.name))
        return;
    if (args[0]) {
        if (!Server.player.isAdmin(chatmsg.sender.name))
            return Server.eBroadcast(Server.lang.unauthorized, chatmsg.sender.name, 'WILD');
        if (args[0].startsWith('@'))
            var player = args.join(' ');
        else {
            if (!Server.player.find(args.join(' ')))
                return Server.eBroadcast(Server.lang.playerDoesNOTexist, chatmsg.sender.name, 'WILD');
            player = `"${args.join(' ')}"`;
        }
    }
    else
        player = `"${chatmsg.sender.name}"`;
    let nx = Math.floor(Math.random() * 10) + 1, nz = Math.floor(Math.random() * 10) + 1, x = 0;
    (x = Server.settings.getScore('wildMax') * Math.random() + Server.settings.getScore('wildMin'));
    x = ~~x;
    let z = 0;
    (z = Server.settings.getScore('wildMax') * Math.random() + Server.settings.getScore('wildMin'));
    z = ~~z, nx = ~~nx, nz = ~~nz;
    if (nx <= 4)
        x = -Math.abs(x);
    if (nz <= 4)
...
Server.command.register(registerInformationWILDADJUST, (chatmsg, args) => {
    ...
    if (max.includes(args[0])) {
        if (isNaN(number))
            return Server.eBroadcast(Server.lang.error2, chatmsg.sender.name, 'WILD SETTINGS');
        if (number < 0)
            return Server.eBroadcast('That number is too small!§8 That\'s what she said XD...', chatmsg.sender.name, 'WILD SETTINGS');
        if (-5000001 > number || number > 5000001)
            return Server.eBroadcast('Yeah, no... No numbers greater than 5 million.', chatmsg.sender.name, 'WILD SETTINGS');
        if (number <= Server.settings.getScore('wildMin'))
            return Server.eBroadcast(`The maximum of ${Server.lang.prefix}wild CANNOT be less than minimum!`, chatmsg.sender.name, 'WILD SETTINGS');
        ...
        if (-5000001 > number || number > 5000001)
            return Server.eBroadcast('Yeah, no... No numbers greater than 5 million.', chatmsg.sender.name, 'WILD SETTINGS');
        if (number >= Server.settings.getScore('wildMax'))
            return Server.eBroadcast(`The minimum of ${Server.lang.prefix}wild CANNOT be greater than maximum!`, chatmsg.sender.name, 'WILD SETTINGS');
