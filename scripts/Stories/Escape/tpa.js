...
Server.command.register({
    ...
    if (!Server.basicToggleChecker('tpa', chatmsg.sender.name))
        return;
    ...
    if (cancel.includes(args[0])) {
        if (!Server.tpa.has(chatmsg.sender.name))
            return Server.eBroadcast('You cannot cancel a TPA that doesn\'t exist!', chatmsg.sender.name, 'TPA');
        Server.tpa.delete(chatmsg.sender.name);
        return Server.broadcast('Your TPA have been §ccanceled§7!', chatmsg.sender.name, 'TPA');
    }
    if (!args[1])
        return Server.eBroadcast(Server.lang.error2, chatmsg.sender.name, 'TPA');
    const player = args.slice(1).join(' ');
    if (player.toUpperCase() === chatmsg.sender.name.toLowerCase())
        return Server.eBroadcast('You cannot TPA yourself!', chatmsg.sender.name, 'TPA');
    if (!Server.player.find(player))
        return Server.eBroadcast(Server.lang.playerDoesNOTexist, chatmsg.sender.name, 'TPA');
    if (accept.includes(args[0])) {
        if (Server.tpa.get(player)?.receiver !== chatmsg.sender.name)
            return Server.eBroadcast(`You either didn't get a TP request frem §4${player}§c or the time has ran out!`, chatmsg.sender.name, 'TPA');
        if (config.tpHere) {
    ...
    if (send.includes(args[0])) {
        if (Server.tpa.has(chatmsg.sender.name))
            return Server.eBroadcast('You cannot send a TP request when you already sent one to somebody. Type "§7!tpa cancel§c" in chat to cancel your previous TP request.', chatmsg.sender.name, 'TPA');
