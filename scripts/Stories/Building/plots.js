...
Server.command.register({
    ...
    if (!plt.has(args[0]))
        return Server.eBroadcast(`This server doesn\'t has the plot "§4${args[0]}§r§c"!`, chatmsg.sender.name, 'PLT');
    const plot = plt.get(args[0]);
    if (!plot.enabled || !chatmsg.sender.hasTag(plot?.tag))
        return Server.eBroadcast('This plot currently isn\'t enabled, or you do not have this plot opened.', chatmsg.sender.name, 'PLT');
    if ((tp.includes(args[1]) || !args[1])) {
        const id = playerID[chatmsg.sender.nameTag];
        if (!id)
            return;
        if (!plot?.useSpawnTeleport && !Server.player.isAdmin(chatmsg.sender.name))
            return Server.eBroadcast(Server.lang.error2, chatmsg.sender.name, 'P');
        let row = plot.rowSize >= 2 ? ~~(id / parseInt(plot.rowSize)) : 0, coords = plot.distance.map((pos, i) => parseInt(plot.startingCoords[i]) + (row >= 1 ? parseInt(plot.nextRow[i]) * row : 0) + (pos * (id - 1)));
    ...
    if (reload.includes(args[1]) && config.letThemReload) {
        const cache = plotCache[chatmsg.sender.name];
...
Server.command.register({
    ...
    if (create.includes(args[0])) {
        if (plt.has(args[1]))
            return Server.eBroadcast(`This server already has the plot "§4${args[1]}§r§c"!`, chatmsg.sender.name, 'Plotz');
        if (args[1].replace(/[a-zA-Z0-9]/g, '') !== '')
            return Server.eBroadcast('You cannot use special characters!', chatmsg.sender.name, 'Plotz');
        if (args[2])
            return Server.eBroadcast('No spaces in the plot name >:((((((!', chatmsg.sender.name, 'Plotz');
        plt.set(args[1], {
            enabled: 0,
    ...
    if (!args.length || list.includes(args[0])) {
        if (args[1]) {
            if (!plt.has(args[1]))
                return Server.eBroadcast(`This server doesn\'t has the plot "§4${args[1]}§r§c"!`, chatmsg.sender.name, 'Plotz');
            const plot = plt.get(args[1]), plotData = [
        ...
        if (!plt.allKeys().length)
            return Server.eBroadcast('You haven\'t set any plots yet!', chatmsg.sender.name, 'Plotz');
        const allPlots = plt.allKeys().map(plot => {
    ...
    if (!args[1])
        return Server.eBroadcast('You need to type a plot name!', chatmsg.sender.name, 'Plotz');
    if (!plt.has(args[1]))
        return Server.eBroadcast(`This server doesn\'t has the plot "§4${args[1]}§r§c"!`, chatmsg.sender.name, 'Plotz');
    const plot = plt.get(args[1]);
    if (del.includes(args[0])) {
        if (args[2] !== 'true')
            return Server.eBroadcast(`Please type "§4true§c" after the plot system's name to comfirm you want to delete the plot system "§4${args[1]}§c" along with all of the member data.`, chatmsg.sender.name, 'ROT');
        for (const player of world.getPlayers()) {
            const cache = plotCache[player.nameTag];
            if (!cache?.includes(plot.tag))
                continue;
            deletePlot(Server.player.getScore('ROTplayerUUID', player.nameTag), [plot.tag]);
            cache.splice(cache.findIndex(tag => plot.tag === tag), 1);
            Object.assign(plotCache, { [player.nameTag]: cache });
        }
        if (plt.has('l-' + plot.id))
            for (const owner of plt.get('l-' + plot.id).split('/r/'))
                plt.delete('p-' + plot.id + '-' + owner);
    ...
    if (set.includes(args[0])) {
        ...
        if (!plt.has(args[1]))
            return Server.eBroadcast(`This server doesn\'t has the plot "§4${args[1]}§r§c"!`, chatmsg.sender.name, 'Plotz');
        const pos = ['pos', 'postion', 'startingpos', 'start', 'startingpostion', 'p', 'startingCoords'], distance = ['d', 'distance', 'length', 'dis', 'dist', 'ds', 'sd'], chatTeleport = ['chattp', 'talktp', 'chatteleport', 'ct'], str = ['structure', 'struc', 'block', 's', 'str'], savingDistance = ['savepos', 'savepostion', 'savpos', 'savd', 'savecoords', 'save'], rowSize = ['rowSize', 'row', 'rs', 'rsize', 'rows'], nextRowAmount = ['nextrowammount', 'nra', 'nr', 'nextrow', 'rowamount', 'nr0'], resetArea = ['resetarea', 'ra', 'leaveArea', 'reset', 'area'], spawnTeleport = ['st', 'spawnteleport', 'spawntp', 'stp', 'jointp', 'jointeleport'], useSpawnTeleport = ['usest', 'usespawnteleport', 'usest', 'ust', 'usespawntp'], friends = ['friends', 'fri', 'people', 'ppl'], tag = ['tag', 't', 'tg', 'tgr'];
        if (distance.includes(args[2])) {
            if (!args[5])
                return Server.eBroadcast('You need to type the distance between each plot', chatmsg.sender.name, 'Plotz');
            let X = parseInt(args[3]), Y = parseInt(args[4]), Z = parseInt(args[5]);
            if (isNaN(X) || isNaN(Y) || isNaN(Z))
                return Server.eBroadcast('That is not a set of coordinates', chatmsg.sender.name, 'Plotz');
        ...
        if (str.includes(args[2])) {
            if (!args[3])
                return Server.eBroadcast(Server.lang.error2, chatmsg.sender.name, 'Plotz');
            if (args[4])
                return Server.eBroadcast('No spaces in the structure name please!', chatmsg.sender.name, 'Plotz');
        ...
        if (pos.includes(args[2])) {
            let X = parseInt(args[3]), Y = parseInt(args[4]), Z = parseInt(args[5]);
            if (!args[3])
                X = ~~chatmsg.sender.location.x, Y = ~~chatmsg.sender.location.y, Z = ~~chatmsg.sender.location.z;
            if (isNaN(X) || isNaN(Y) || isNaN(Z))
                return Server.eBroadcast('That is not a set of coordinates!', chatmsg.sender.name, 'Plotz');
        ...
        if (savingDistance.includes(args[2])) {
            if (!args[5])
                return Server.eBroadcast('You need to type the how many blocks in the X-Y-Z format will be saved on the plot', chatmsg.sender.name, 'Plotz');
            let X = parseInt(args[3]), Y = parseInt(args[4]), Z = parseInt(args[5]);
            if (isNaN(X) || isNaN(Y) || isNaN(Z) || X > 64 || Z > 64)
                return Server.eBroadcast('That is not a set of coordinates or either the X or Z value is greater than §464', chatmsg.sender.name, 'Plotz');
        ...
        if (rowSize.includes(args[2])) {
            if (isNaN(parseInt(args[3])))
                return Server.eBroadcast('That is not a number!', chatmsg.sender.name, 'Plotz');
            if (parseInt(args[3]) === 1)
                return Server.eBroadcast('The the row size cannot be one!', chatmsg.sender.name, 'Plotz');
        ...
        if (nextRowAmount.includes(args[2])) {
            if (!args[5])
                return Server.eBroadcast('You need to type the how many blocks away in the X-Y-Z format will the next row generate', chatmsg.sender.name, 'Plotz');
            let X = parseInt(args[3]), Y = parseInt(args[4]), Z = parseInt(args[5]);
            if (isNaN(X) || isNaN(Y) || isNaN(Z) || X > 64 || Z > 64)
                return Server.eBroadcast('That is not a set of coordinates', chatmsg.sender.name, 'Plotz');
        ...
        if (tag.includes(args[2])) {
            ...
            if (args[4])
                return Server.eBroadcast('No spaces in the tag name please!', chatmsg.sender.name, 'Plotz');
    ...
    if (toggle.includes(args[0])) {
        ...
        if (!plt.has(args[1]))
            return Server.eBroadcast(`This server doesn\'t has the plot "§4${args[1]}§r§c"!`, chatmsg.sender.name, 'Plotz');
        if (!plot.distance)
            return Server.eBroadcast(`The §4${args[1]}§c's spawning distance between each plot has not been set! You can set it by typing "§4!plot set ${args[1]} distance <X> <Y> <Z>§c" or whatever number you want.`, chatmsg.sender.name, 'Plotz');
        if (!plot.structure)
            return Server.eBroadcast(`§4${args[1]}§c's base structure is not set! You can set it by typing "§4!plot set ${args[1]} str <structure name>§c".`, chatmsg.sender.name, 'Plotz');
        if (!plot.startingCoords)
            return Server.eBroadcast(`§4${args[1]}§c's starting postion is not set! You can set it by typing "§4!plot set ${args[1]} pos <X> <Y> <Z>§c".`, chatmsg.sender.name, 'Plotz');
        if (!plot.savingDistance)
            return Server.eBroadcast(`§4${args[1]}§c's saving distances is not set! You can set it by typing "§4!plot set ${args[1]} savd <X> <Y> <Z>§c".`, chatmsg.sender.name, 'Plotz');
    ...
    if (clear.includes(args[0])) {
        if (!args[2])
            return Server.broadcast(`You didn\'t type a players name meaning you might want to delete all the player data. Type "§c!plot clear ${args[1]} ${plot.id}§7"`, chatmsg.sender.name, 'Plotz');
        ...
        if (!isNaN(parseInt(args[2])))
            return Server.eBroadcast('Did you type the correct reset number?', chatmsg.sender.name, 'Plotz');
        if (!Server.player.find(args.slice(2).join(' ')))
            return Server.eBroadcast('Sorry, the member you typed in either doesn\'t exist or isn\'t online. You can only clear online players at the moment.', chatmsg.sender.name, 'Plotz');
        const cache = plotCache[chatmsg.sender.name];
        deletePlot(Server.player.getScore('ROTplayerUUID', chatmsg.sender.name), [plot.tag]);
        cache.splice(cache.findIndex(tag => plot.tag === tag), 1);
        Object.assign(plotCache, { [chatmsg.sender.name]: cache });
...
setTickInterval(() => {
    ...
    for (const player of world.getPlayers()) {
        ...
        if (!id || !playerUUID)
            return;
        plotCache[player.nameTag]?.forEach((tag, i) => {
            if (Server.player.findTag(tag, player.nameTag))
                return;
            const cache = plotCache[player.nameTag];
        ...
        playerPlots.forEach(plot => {
            const row = plot.rowSize >= 2 ? ~~(id / parseInt(plot.rowSize)) : 0, coords = plot.distance.map((pos, i) => parseInt(plot.startingCoords[i]) + (row >= 1 ? parseInt(plot.nextRow[i]) * row : 0) + (pos * (id - 1)));
            if (!plt.has('p-' + plot.id + '-' + playerUUID)) {
                ...
                Server.runCommand(`structure load "${plot.id + '-' + playerUUID}" ` + coords.join(' '));
            ...
            if (plot?.useSpawnTeleport)
                Server.player.fetch(player.nameTag).teleport(new Location(coords[0] + plot?.spawnTeleport[0], coords[1] + plot?.spawnTeleport[1], coords[2] + plot?.spawnTeleport[2]), world.getDimension('overworld'), 0, 0);
            savePlot(playerUUID);
        });
    }
}, 20);
 ...
 */
const savePlot = (playerUUID, plotTag) => plt.allValues()?.filter(plot => plotTag?.includes(plot.tag) ?? true)?.forEach(plot => {
    if (!plot.enabled || !plt.has('p-' + plot.id + '-' + playerUUID))
        return;
    const id = plt.get('p-' + plot.id + '-' + playerUUID).id, row = plot.rowSize >= 2 ? ~~(id / plot.rowSize) : 0, coords = plot.distance.map((pos, i) => plot.startingCoords[i] + (row >= 1 ? plot.nextRow[i] * row : 0) + (pos * (id - 1)));
 ...
 */
deletePlot = (playerUUID, plotTag) => plt.allValues()?.filter(plot => plotTag?.includes(plot.tag) ?? true)?.forEach(plot => {
    if (!plot.enabled || !plt.has('p-' + plot.id + '-' + playerUUID))
        return;
