...
world.beforeEvents.chatSend.subscribe(data => {
    try {
        ...
        if (!data.message.startsWith(quick.prefix)) {
            ...
            const message = data.message.charAt(0).toUpperCase() + data.message.slice(1), rank = `§7[${Player.getPrefixes(data.sender).join('§r§7, ')}§r§7] ${Player.getNameColor(data.sender)}`;
