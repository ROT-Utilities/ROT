...
export const updateLang = () => Object.assign(Lang, {
    ...
    cmd: {
        unknown: `Unknown command! Make sure you have permission to use that command and make sure you spelt it correctly. Type "§6${quick.prefix}help§e" in chat for a list of available commands. "§7/tag @s add v§e" will give you admin permissions.`,
        ...
        notAArg: (cmd, before, err, afther, tip) => `Not sure what you mean by "§6${quick.prefix + cmd}§e${before.length ? ` ${before.join(' ')}` : ''}§r §g${err}§r§6${afther ? ` §g${afther}` : ''}§r§e".... §g${err[0].toUpperCase() + err.slice(1)}§r§e is not a vaild argument.${tip ? ` Maybe try typing §a${tip}§e?` : ''}`,
        noPerms: 'You do not have permission to execute this command. "§7/tag @s add v§e" will give you admin permissions.§r',
