...
export class ChestGui {
    ...
    setType(type) {
        ...
        const size = ChestGuiTypes[type] ?? ChestGuiTypes['small'];
        this.size = size[1];
    ...
    item(slot, name, lore, icon, stackSize = 1, enchanted = false, enchants) {
        ...
        let enchantments = enchants.map(e => `§7${beauty('minecraft:' + e.id)} ${toRomanNumeral(e.level)}`);
        //@ts-ignore
    ...
    show(player) {
        ...
        this.formData.items.forEach((item) => form.button({ translate: item.data }, item.icon?.toString()));
