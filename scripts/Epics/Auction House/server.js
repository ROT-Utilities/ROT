...
import { confirmForm, getPost, openAH } from "./main.js";
...
export async function serverPosts(player, from) {
    ...
    if (!allPosts.length)
        return await confirmForm(player, '§8§lCreate a new auction?§r', 'Huh... There are no auctions going on right now. Do you want to make one?') ? createPost(player, openAH) : openAH(player);
