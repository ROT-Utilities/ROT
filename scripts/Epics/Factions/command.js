...
cmd.dynamicType('home', ['home', 'spawn', 'h'], plr => {
    if (!has({ player: plr }))
        return plr.error('You aren\'t even in a faction!');
    const spawn = Database.register(fac.player.read(plr.rID, true), 'FTN').read('s');
    if (!spawn.length)
        return plr.error('You faction spawn hasen\'t been set yet...');
    plr.send('You have been teleported to your faction\'s spawnpoint');
    plr.teleport({ x: spawn[0], y: spawn[1], z: spawn[2] }, world.getDimension(spawn[3]), plr.getRotation().x, plr.getRotation().y);
});
//set:name
cmd.dynamicType('info', ['info', 'information'], plr => {
...
cmd.dynamicType('members', ['members', 'member', 'mem', 'm'], plr => {
    let owner = config.veiwFaction ? getOwner(getChunk(plr)) : undefined;
    if (!owner)
        owner = fac.player.read(plr.rID, true);
    if (!owner || !Database.has(owner, 'FTN'))
...
cmd.dynamicType('leave', ['leave', 'l'], plr => {
    if (!has({ player: plr }))
        return plr.error('You aren\'t even in a faction!');
    const db = Database.register(fac.player.read(plr.rID, true), 'FTN');
...
cmd.dynamicType('kick', ['kick'], (plr, _, args) => {
    if (!has({ player: plr }))
        return plr.error('You aren\'t even in a faction!');
    if (fac.invites.find(i => fac.player.read(plr.rID, true) === i[0] && i[2] === args[0].name))
        return plr.error('You already sent a invite to this player.');
