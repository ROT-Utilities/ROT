...
export function claimChunk(player) {
    ...
    if (db.read('p') < config.minFacClaimLand)
        return player.send(`Your faction doesn't have the required average power. You have §6${db.read('p')} §epower, §6${config.minFacClaimLand} §epower is required.`);
    ...
    const chunk = getChunk(player), ownerID = getOwner(chunk);
    ...
    if (!ownerID) {
        ...
        const ownerDB = Database.register(ownerID, 'FTN');
        if (config.mostPowerClaims && ownerDB.read('p') > db.read('p'))
            return player.send('You do not have enough power to overwrite this faction\'s claim.');
        if (!config.mostPowerClaims && ownerDB.read('p') >= config.minFacClaimLand)
