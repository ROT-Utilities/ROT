...
system.runInterval(() => {
    if (config.tag.length)
        world.getAllPlayers().forEach(player => {
            ...
            main(Player.playerType(player, { from: config.npcName, sound: false }));
        });
    ...
    for (const dim of ['overworld', 'nether', 'the end'])
        Array.from(world.getDimension(dim).getEntities({ type: 'rot:shop' })).forEach(entity => entity.nameTag = name);
}, 5);
