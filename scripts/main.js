...
import { EntityQueryOptions, Location, world, ItemStack, MinecraftEnchantmentTypes, MinecraftItemTypes } from 'mojang-minecraft';
import { updateLeaderboard, getRanks, getColors, setTickInterval, twoStepKick } from './Papers/paragraphs/ExtrasParagraphs.js';
...
*/
export let plotCache = {}, playerID = {};
let ready = false, totalTick = 0, mines = {}, memberJoin = [], allEnchantments = Object.values(MinecraftEnchantmentTypes);
...
setTickInterval(() => {
    ...
    if (!Server.settings.getScore('ROT'))
        return;
    // Statics
    ...
    if (memberJoin.length) {
        ...
        memberJoin.forEach(member => {
            if (!Server.player.list().includes(member))
                return;
            memberJoin.splice(memberJoin.findIndex(name => name === member), 1);
            //if(member.replace(/[a-zA-Z0-9 ]/g, '') !== '') return twoStepKick(Server.player.fetch(member));
            Server.player.list().forEach((_, i) => {
                if (Object.values(playerID).includes(i + 1))
                    return;
            ...
            if (Object.keys(banUUIDs).includes(member) || Object.values(banUUIDs).includes(Server.player.getScore('ROTplayerUUID', member))) {
                const banData = Object.keys(banUUIDs).includes(member) ? bannedPlayers[member] : bannedPlayers[Object.values(banUUIDs).find(UUID => UUID === Server.player.getScore('ROTplayerUUID', member))];
                if (Server.bans.has(banData?.bannedPlayer))
                    twoStepKick(Server.player.fetch(member), banData);
            }
            if (config.joinTeleport)
                Server.player.fetch(member).teleport(new Location(Server.settings.get('spawn')[0], Server.settings.get('spawn')[1], Server.settings.get('spawn')[2]), world.getDimension(Server.settings.get('spawn')[3]), 0, 0);
            ...
            if (config.joinMessage)
                if (!Server.player.getScore('ROTnftj', member)) {
                    ...
                    Server.broadcast(Server.lang.welcomeback, member, 'ROT');
            if (config.fireworksAgain)
                Server.runCommand(`execute "${member}" ~~~ summon fireworks_rocket ~~~`);
        });
    }
    for (const player of world.getPlayers()) {
        ...
        if (config.anti32k && !player.hasTag(config.adminTag)) {
            ...
            for (let i = 0; i < inventory.size; i++) {
                ...
                if (!item)
                    continue;
                if (config.bannedItems.includes(item.id))
                ...
                allEnchantments.forEach(ench => {
                    if (itemEnchantments.hasEnchantment(ench) < (ench.maxLevel + config.maxEnchant))
                        return;
                    inventory.setItem(i, new ItemStack(MinecraftItemTypes.air));
                });
            }
    ...
    if (Server.settings.getScore('tpa')) {
        ...
        if (allTPAs)
            for (let key in allTPAs)
                if (allTPAs.hasOwnProperty(key) && allTPAs[key].sender && allTPAs[key].timeLeft < Date.now()) {
                    Server.eBroadcast(`The TPA you sent to §4${allTPAs[key].sender}§c has expired!`, allTPAs[key].sender, 'TPA');
                    Server.tpa.delete(key);
                }
    }
    if (Server.settings.getScore('sleep'))
        Server.runCommand('execute @a[tag=is_sleeping] ~~~ time add 100');
    if (Server.settings.getScore('health'))
        Server.runCommand('scoreboard objectives setdisplay belowname ROTh');
    ...
    if (config.forceAdmin)
        if (Server.player.isAdmin(RP) && !config.forcedAdmins.includes(RP)) {
            Server.bans.set(RP, {
                bannedPlayer: RP,
    ...
    if (Server.player.findTag('afk', RP) && Server.settings.getScore('AFK')) {
        ...
        if (Server.player.getScore('ROTafk', RP) === Math.round(Server.settings.getScore('kickTime') / 2))
            Server.player.isTrusted(RP) ? Server.broadcast(`§c${RP}§7 is §e§lAFK§r§7! Tell them to come back! They are wasting §4§lROT's§r§7 resources! Ahhhhhhhh!`, '@a', 'ROT') : Server.broadcast(`§c${RP}§7 is §e§lAFK§r§7!` + (config.kickAFK ? ` Tell them to come back or they will be kicked in §c§l${Math.round(Server.settings.getScore('kickTime') / 2)}§r§7 seconds!` : ''), '@a', 'ROT');
    ...
    if (Server.settings.getScore('KTT')) {
        ...
        if (!Server.player.isAdmin(RP) && ((!Server.settings.getScore('KTR') && Server.player.findTag(tag, RP)) || (!Server.player.findTag(tag, RP))))
            Server.runCommand(`kick "${RP}" ` + reason);
    }
    for (let tag of Server.player.getTags(RP)) {
        if (Server.sc.has(tag)) {
            ...
            if (/(?<=\(score:).+?(?=\))/.test(board))
                board.match(/(?<=\(score:).+?(?=\))/g).map((obj) => board = board.replace(`(score:${obj})`, Server.player.getScore(obj, RP) ? metricNumbers(Server.player.getScore(obj, RP)) : 0));
            Server.runCommand(`titleraw "${RP}" title {"rawtext":[{"text":${JSON.stringify(board)}}]}`);
        }
        if (config.ROTCommandTag && tag.startsWith('ROT')) {
            let fullCommand = tag.match(/(?<=ROT).*/)[0], args = fullCommand.trim().split(/\s+/), command = args.shift().toLowerCase(), commandREG = Server.command.getRegistration(command), isAdmin = false;
            if (Server.player.isAdmin(RP))
                isAdmin = true;
            ...
            if (!isAdmin)
                Server.runCommand(`tag "${RP}" remove ` + config.adminTag);
        }
    }
}, 20);
...
if (config.antiNuker)
    world.events.blockBreak.subscribe(({ block, brokenBlockPermutation, dimension, player }) => {
        ...
        if (!vegetation.includes(block.id))
            return;
        Object.assign(mines, { [player.name]: Date.now() });
        if (mine < (Date.now() - 50) || player.hasTag(config.adminTag))
            return;
...
// @ts-ignore
if (config.bannedItems?.length)
    world.events.beforeItemUseOn.subscribe(data => !data.source.hasTag(config.adminTag) && config.bannedBlocks.includes(data.item.id) && (data.cancel = true));
...
if (config.debug)
    import('./Stories/ROT/debug.js');
if (config.prefix)
    import('./Stories/ROT/prefix.js');
if (config.reset)
    import('./Stories/ROT/reset.js');
if (config.db)
    import('./Stories/ROT/db.js');
//Server
if (config.Server)
    import('./Stories/Server/Main.js');
if (config.broadcast)
    import('./Stories/Server/broadcast.js');
if (config.close)
    import('./Stories/Server/close.js');
if (config.members)
    import('./Stories/Server/members.js');
if (config.clearchat)
    import('./Stories/Server/clearchat.js');
if (config.scoreboard)
    import('./Stories/Server/scoreboard.js');
if (config.discord)
    import('./Stories/Server/discord.js');
if (config.realm)
    import('./Stories/Server/realm.js');
//Management
...
if (config.mute)
    import('./Stories/Management/mute.js');
if (config.ranks)
    import('./Stories/Management/ranks.js');
if (config.colors)
    import('./Stories/Management/colors.js');
if (config.inventory)
    import('./Stories/Management/inventory.js');
if (config.kicktags)
    import('./Stories/Management/kicktags.js');
if (config.AFK)
    import('./Stories/Management/AFK.js');
if (config.timeout)
    import('./Stories/Management/timeout.js');
if (config.tac)
    import('./Stories/Management/tac.js');
//Miscellaneous
if (config.Miscellaneous)
    import('./Stories/Miscellaneous/Main.js');
if (config.smyte)
    import('./Stories/Miscellaneous/smyte.js');
if (config.nothing)
    import('./Stories/Miscellaneous/nothing.js');
if (config.privatechat)
    import('./Stories/Miscellaneous/privatechat.js');
if (config.timezone)
    import('./Stories/Miscellaneous/timezone.js');
if (config.dimension)
    import('./Stories/Miscellaneous/dimension.js');
if (config.lore || config.click)
    import('./Stories/Miscellaneous/item.js');
if (config.sudo)
    import('./Stories/Miscellaneous/sudo.js');
//Escape
if (config.Escape)
    import('./Stories/Escape/Main.js');
if (config.warp)
    import('./Stories/Escape/warp.js');
if (config.pwarp)
    import('./Stories/Escape/pwarp.js');
if (config.cmd)
    import('./Stories/Escape/cmd.js');
if (config.feed)
    import('./Stories/Escape/feed.js');
if (config.heal)
    import('./Stories/Escape/heal.js');
if (config.top)
    import('./Stories/Escape/top.js');
if (config.kill)
    import('./Stories/Escape/kill.js');
if (config.vanish)
    import('./Stories/Escape/vanish.js');
if (config.health)
    import('./Stories/Escape/health.js');
if (config.wild)
    import('./Stories/Escape/wild.js');
if (config.home)
    import('./Stories/Escape/home.js');
if (config.spawn)
    import('./Stories/Escape/spawn.js');
if (config.tpa)
    import('./Stories/Escape/tpa.js');
if (config.sleep)
    import('./Stories/Escape/sleep.js');
if (config.GMS)
    import('./Stories/Escape/GMS.js');
if (config.GMC)
    import('./Stories/Escape/GMC.js');
if (config.GMA)
    import('./Stories/Escape/GMA.js');
if (config.GMA)
    import('./Stories/Escape/repair.js');
//Building
if (config.Building)
    import('./Stories/Building/Main.js');
if (config.text)
    import('./Stories/Building/text.js');
if (config.leaderboard)
    import('./Stories/Building/leaderboard.js');
if (config.plot)
    import('./Stories/Building/plots.js');
if (config.region)
    import('./Stories/Building/region.js');
//Fantasy
if (config.Fantasy)
    import('./Stories/Fantasy/Main.js');
if (config.rickroll)
    import('./Stories/Fantasy/rickroll.js');
if (config.rottle)
    import('./Stories/Fantasy/rottle.js');
//Other data
const vegetation = [
