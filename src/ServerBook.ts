...
class NewServerBook extends ServerPaper {
    ...
    public lang = new LangPaper();
    //Databases
    public settings = new DatabasePaper('SS');
    public bans = new DatabasePaper('BS');
    public mutes = new DatabasePaper('MS');
    public sc = new DatabasePaper('SC');
    public tpa = new DatabasePaper('TPA');
    public gens = new DatabasePaper('GNS')
    constructor() {
        super();
        this._buildEvent();
    }
    ...
    private _buildEvent() {
        world.events.beforeChat.subscribe(data => {
            ...
            if(!data.message.startsWith(this.lang.prefix)) {
                if(!this.settings.getScore('ROT')) return data.cancel = false;
                if(data.message.startsWith('!')) return this.eBroadcast(`Sorry, the prefix on this server is now "§7${this.lang.prefix}§c"`, data.sender.name, 'ROT');
                if(this.player.findTag('mute', data.sender.name)) return this.eBroadcast('Sorry, you have been muted by ROT or an admin', data.sender.name, 'CHAT MESSAGE');
                if(data.message.trim() === '') return;
                if(config.antiSpam && !this.player.isTrusted(data.sender.name)) this.runCommand(`scoreboard players add "${data.sender.name}" ROTMessageMute 1`);
                ...
                for(const player of this.player.list().filter(member => this.player.getScore('ROTPrivateChat', data.sender.name) === Server.player.getScore('ROTPrivateChat', member))) {
                    ...
                    if(currentHour <= 12) AMPM = currentHour === 12 ? ' PM' : ' AM', currentHour === 0 ? currentHour = 12 : null; else currentHour = currentHour - 12, AMPM = ' PM';
                    this.broadcast(
                        (this.player.getScore('ROTPrivateChat', data.sender.name) > 0 ? '§a§lPRIVATE§r §e- ' : '')
                        + RankName
            ...
            const args = data.message.slice(this.lang.prefix.length).trim().split(/\s+/),
            ...
            this.command.getAllRegistation().forEach(element => {
                if(element.name !== command && !element?.aliases?.includes(command)) return;
                /**
                 * Registration callback
                 */
                if(element?.cancelMessage) data.cancel = true;
                if(this.settings.getScore('ROT') !== 1 && command !== 'setup') return this.eBroadcast(this.lang.setupv, data.sender.name, 'ROT');
                if(element?.admin && !this.player.isAdmin(data.sender.name)) return this.eBroadcast(this.lang.error, data.sender.name, 'ROT');
                ...
                try {
                    ...
                    if(config.tellTime) this.broadcast(`Command executed in §c${Date.now() - postCMDTime}§7 MS!`, data.sender.name, 'ROT');
                }
            });
        })
    }
    ...
    * @returns The command status to the Server.settings database
     */
    basicCommandToggle(command: string, player: string): void {
        if(!this.settings.getScore(command)) this.settings.setScore(command, 0);
        if(this.settings.getScore(command) === 0) return this.broadcast(this.lang.CMDon(command), player), this.settings.setScore(command, 1);
        if(this.settings.getScore(command) === 1) return this.broadcast(this.lang.CMDTrusted(command), player), this.settings.setScore(command, 2);
        if(this.settings.getScore(command) === 2) return this.broadcast(this.lang.CMDTag(command), player), this.settings.setScore(command, 3);
        if(this.settings.getScore(command) === 3) this.broadcast(this.lang.CMDoff(command), player), this.settings.setScore(command, 0);
    }
    /**
     ...
     * @returns {boolean}
     */
    basicToggleChecker(command: string, player: string): boolean {
        if(!this.settings.getScore(command)) this.settings.setScore(command, 0);
