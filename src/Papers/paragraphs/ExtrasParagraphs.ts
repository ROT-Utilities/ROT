...
Thank you!
*/
import { world, Entity, Player, Location, BlockLocation } from 'mojang-minecraft';
...
*/
getColors = (player: string) => config.defaultNameColor + (new PlayerPaper().getTags(player)?.filter(tag => tag.startsWith('color:'))?.map(color => color.replace('color:', '').trim()).join('') ?? ''),
/**
* @function getChat Gets the chat color of a player
* @param {string} player the player
* @example getChat(Mo9ses);
*/
getChat = (player: string) => config.defaultChatColor + (new PlayerPaper().getTags(player)?.filter(tag => tag.startsWith('chat:'))?.map(color => color.replace('chat:', '').trim()).join('') ?? ''),
/**
 ...
 * @returns {boolean}
 */
betweenXYZ = (XYZa: number[], XYZb: number[], XYZc: number[]) => XYZc.length === XYZc.filter((c: number, i: number) => (c >= Math.min(XYZa[i], XYZb[i])) && (c <= Math.max(XYZa[i], XYZb[i]))).length,
/**
 ...
 */
isReach = (pos1: Location | BlockLocation, pos2: Location | BlockLocation) => Math.sqrt((pos1.x - pos2.x) ** 2 + (pos1.y - pos2.y) ** 2 + (pos1.z - pos2.z) ** 2) > 7,
/**
