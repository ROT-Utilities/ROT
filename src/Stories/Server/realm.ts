...
Server.command.register({
    ...
    if(args[0] && chatmsg.sender.hasTag(config.adminTag)) return Server.settings.set('realmCode', args.join(' ')), Server.broadcast(`§cThe realm code is now: §d` + Server.settings.get('realmCode'), chatmsg.sender.name, 'ROT');
