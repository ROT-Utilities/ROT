...
config.click && Server.command.register({
    ...
    if(!args[1] || !args[1].includes(':') || args[1].replace(/[a-zA-Z0-9]:?/g, '') !== '') return Server.eBroadcast('Type a item ID "insert skull emoji here".', chatmsg.sender.name, 'click');
    const place = ['c', 'click', 'cli', 'cl', 'u', 'use', 'us', 'place', 'plc', 'pl'],
        hit = ['p', 'pun', 'punch', 'h', 'hit'],
        sync = ['s', 'sync', 'sy', 'same', 'both'];
    if(set.includes(args[0])) {
        if(!args[2] || !args[3]) return Server.eBroadcast(`You need to type a command you want this action to run. If you want to remove the item just do "§c!click remove <item>§7"`, chatmsg.sender.name, 'click');
    ...
    if(del.includes(args[0])) {
        ...
        if(place.includes(args[2])) {
            ...
            if(!commands.c) return Server.eBroadcast('The click command was never set!', chatmsg.sender.name, 'click');
            delete commands['c'];
            if(!commands.p) click.delete(args[1]); else click.set(args[1], commands);
            return Server.broadcast(`The item "§c${args[1]}§7"'s click command has been §4§lDeleted!`, chatmsg.sender.name, 'click');
        }
        if(hit.includes(args[2])) {
            const commands = click.get(args[1]);
            if(!commands.p) return Server.eBroadcast('The hit command was never set!', chatmsg.sender.name, 'click');
            delete commands['p'];
            if(!commands.c) click.delete(args[1]); else click.set(args[1], commands);
...
if(config.click) {
    world.events.beforeItemUse.subscribe(data => {
        if(!click.has(data.item.id)) return;
    ...
    world.events.entityHit.subscribe(data => {
        ...
        if(!item || !click.has(item.id)) return;
...
config.lore && Server.command.register({
    ...
    if(!item) return Server.eBroadcast('You need to hold an item!', chatmsg.sender.name, 'Lore');
