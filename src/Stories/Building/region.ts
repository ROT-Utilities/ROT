...
Server.command.register({
    ...
    if(create.includes(args[0])) {
        if(!args[1]) return Server.eBroadcast('You need to type a region name!', chatmsg.sender.name, 'Region');
        if(reg.has(args[1])) return Server.eBroadcast(`This server already has the region "§4${args[1]}§r§c"!`, chatmsg.sender.name, 'Region');
        if(args[1].replace(/[a-zA-Z0-9]/g, '') !== '') return Server.eBroadcast('You cannot use special characters!', chatmsg.sender.name, 'Region');
        if(args[2]) return Server.eBroadcast('No spaces in the region name please!', chatmsg.sender.name, 'Region');
        reg.set(args[1], {
            enabled: 0,
            tag: args[1]
        }, true);
    ...
    if(!args.length || list.includes(args[0])) {
        if(args[1]) {
            if(!reg.has(args[1])) return Server.eBroadcast(`The server doesn't have a region with the name "§4${args[1]}§r§c"!`, chatmsg.sender.name, 'Region');
        ...
        if(!reg.allKeys().length) return Server.eBroadcast('You haven\'t set any regions yet!', chatmsg.sender.name, 'Region');
        const allRegions: any = reg.allKeys().map(regi => {
    ...
    if(set.includes(args[0])) {
        ...
        if(pos1.includes(args[2])) {
            ...
            // @ts-ignore
            if(!args[3]) X = parseInt(chatmsg.sender.location.x), Y = parseInt(chatmsg.sender.location.y), Z = parseInt(chatmsg.sender.location.z);
            if(isNaN(X)||isNaN(Y)||isNaN(Z)) return Server.eBroadcast('That is not a set of coordinates!', chatmsg.sender.name, 'Region');
        ...
        if(tag.includes(args[2])) {
            ...
            if(args[4]) return Server.eBroadcast('No spaces in the tag name please!', chatmsg.sender.name, 'Region');
    ...
    if(toggle.includes(args[0])) {
        if(!region.pos1) return Server.eBroadcast(`§4${args[1]}§c's postion 1 is not set! You can set it by typing "§4!reg set ${args[1]} pos1§c" in chat.`, chatmsg.sender.name, 'Region');
        if(!region.pos2) return Server.eBroadcast(`§4${args[1]}§c's postion 2 is not set! You can set it by typing "§4!reg set ${args[1]} pos2§c" in chat.`, chatmsg.sender.name, 'Region');
...
world.events.blockBreak.subscribe(({ block, brokenBlockPermutation, dimension, player }) => !player?.hasTag(config.adminTag) && reg.allValues()?.forEach(region => {
    if(!region.enabled || region.mine || player.hasTag(region.tag) || !betweenXYZ([...region.pos1], [...region.pos2], [block.location.x, block.location.y, block.location.z])) return;
...
world.events.beforeItemUseOn.subscribe(data => !data?.source?.hasTag(config.adminTag) && reg.allValues()?.some(region => {
    if(!region.enabled || region.build || data.source.hasTag(region.tag) || !betweenXYZ([...region.pos1], [...region.pos2], [data.blockLocation.x, data.blockLocation.y, data.blockLocation.z])) return;
...
world.events.entityHit.subscribe(({ entity, hitEntity }) => hitEntity && reg.allValues()?.forEach(region => {
    if(!region.enabled || region.pvp || !betweenXYZ([...region.pos1], [...region.pos2], [hitEntity.location.x, hitEntity.location.y, hitEntity.location.z])) return;
