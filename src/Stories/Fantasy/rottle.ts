...
Server.command.register({
    ...
    if(!rottle.has(chatmsg.sender.name)) rottle.set(chatmsg.sender.name, {inMatch: false});
    if(play.includes(args[0])) {
        if(rottle.get(chatmsg.sender.name).inMatch) return Server.eBroadcast('You cannot start a game with me if you are already in a game with me! Type "§4!rm quit§c" in chat and give me the win loser', chatmsg.sender.name, 'ROTLE');
    ...
    if(tryo.includes(args[0])) {
        if(!rottle.get(chatmsg.sender.name).inMatch) return Server.eBroadcast('You are not in a match with me yet... Type "§4!rm play§c" in chat to start one.', chatmsg.sender.name, 'ROTLE');
        if(args[1].replace(/[a-z]/g, '') !== '' || args[2]) return Server.eBroadcast('It\'s only one word, and the word doesn\'t have any numbers or stuff like that. Don\'t worry, I didn\'t count that as a "attempt"', chatmsg.sender.name, 'ROT');
        ...
        if(rottle.get(chatmsg.sender.name).attempts >= config.rottleAttemps) {
            ...
            if(config.rottleRewards) Server.runCommands(config.rottleRewardsCmds.map(cmd => {return cmd.replace('@rottler', `"${chatmsg.sender.name}"`)}))
